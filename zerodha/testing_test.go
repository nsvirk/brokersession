...
// defaultTestClient returns a *http.Client configured the same way the
// production code does (cookie jar, manual-redirect handler) so tests
// exercise the production HTTP plumbing.
...
// pointAllToServer redirects every Zerodha flow URL to srv.URL with the
// same path component as the production URL, so a single httptest.Server
// can route every step via one mux.
